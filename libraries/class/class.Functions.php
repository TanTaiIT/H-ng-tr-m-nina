	...
	class Functions
	{
		...
		public function formenu($table,$type,$showh2=1)
		{
			...
			for($i=0;$i<count($danhmuc_cap1);$i++){
					$str.='<li><span></span><a href="'.$danhmuc_cap1[$i][$sluglang].'">'.$start_h2.$danhmuc_cap1[$i]["ten"].$end_h2.'</a>';
					...
					if(count($danhmuc_cap2)>0){
						$str.='<ul>';
							for($j=0;$j<count($danhmuc_cap2);$j++){
								$str.='<li><span></span><a href="'.$danhmuc_cap2[$j][$sluglang].'">'.$start_h2.$danhmuc_cap2[$j]["ten"].$end_h2.'</a>';
									...
									if(count($danhmuc_cap3)>0){
										$str.='<ul>';
											for($k=0;$k<count($danhmuc_cap3);$k++){
												...
												$str.='<li><span></span><a href="'.$danhmuc_cap3[$k][$sluglang].'">'.$start_h2.$danhmuc_cap3[$k]["ten"].$end_h2.'</a>';
												if(count($danhmuc_cap4)>0){
													$str.='<ul>';
														for($h=0;$h<count($danhmuc_cap4);$h++){
															
															$str.='<li><span></span><a href="'.$danhmuc_cap4[$h][$sluglang].'">'.$start_h2.$danhmuc_cap4[$h]["ten"].$end_h2.'</a>';															
														}
													$str.='</ul>';
												}											
											}
		...
		public function formenu1($table,$type,$showh2=1)
		{
			global $config_base,$lang,$sluglang;
			$sqlc1="SELECT ten$lang as ten, tenkhongdauvi, tenkhongdauen, id FROM table_".$table." WHERE hienthi=1 AND type ='".$type."' ORDER BY stt,id DESC";
			$arr_sqlc1=array();
			$danhmuc_cap1 = $this->d->rawQuery($sqlc1,$arr_sqlc1);
		...
		public function formenubv($table,$type)
		{
			...
			$str.='<ul>';
			for($i=0;$i<count($danhmuc_cap1);$i++){
					$str.='<li><span></span><a href="'.$danhmuc_cap1[$i][$sluglang].'"><h2>'.$danhmuc_cap1[$i]["ten"].'</h2></a></li>';
			}
			$str.='</ul>';		
			return $str;
		}
		...
		/*--- Lấy sản phẩm ----*/
		function lay_sanpham($truyvan){
				global $lang,$sluglang,$config;
				$str = "";
				$nametype = 'san-pham';
				$dongdau = THUMBS.'/264x240x1/';
				...
				foreach($truyvan as $k => $v){
					$str .='<div class="product">
			            <a class="box-product text-decoration-none" href="'.$v[$sluglang].'" title="'.$v['ten'.$lang].'">
			                <p class="pic-product scale-img"><img onerror="this.src=\''.$dongdau.'assets/images/noimage.png\'" src="'.$dongdau.UPLOAD_PRODUCT_L.$v['photo'].'" alt="'.$v['ten'.$lang].'"/></p>
			                <h3 class="name-product text-split">'.$v['ten'.$lang].'</h3>
			                <p class="price-product">';
			                    if($v['giamoi']!=0 && $v['gia']!=0) {
			                        $str .='<div class="pr"><p><span class="price-new">'.number_format($v['giamoi'],0, ',', '.').'đ</span></p>
			                        <span class="price-old">'.number_format($v['gia'],0, ',', '.').'đ</span>
			                        <p><a class="chitiet" href="'.$v['tenkhongdau'.$lang].'">Chi tiết</a></p>
			                        </div>
			                        <span class="price-per">-'.round(100-($v['giamoi']/$v['gia']*100)).'%</span>';
			                        //<span class="price-per">-'.$v['giakm'].'%</span>
			                    } else {

			                    	if($v['gia']!=0) $s_gia = number_format($v['gia'],0, ',', '.').'đ';else $s_gia = lienhe;
			                        $str .='<span class="price-new">'.$s_gia.'</span>';
			                    }
			                $str .='</p>
			            </a>';
		...
		/*--- Lấy baiviet ----*/
		function lay_baiviet($truyvan,$cssitem){
				global $lang,$sluglang;				
				$thumb = THUMBS.'/160x120x1/';				
				foreach($truyvan as $k => $v){ ?>
					<a class="<?=$cssitem?> text-decoration-none w-clear" href="<?=$v[$sluglang]?>" title="<?=$v['ten'.$lang]?>">
			            <p class="pic-news scale-img"><img onerror="this.src='<?=$thumb?>assets/images/noimage.png';" src="<?=$thumb?><?=UPLOAD_NEWS_L.$v['photo']?>" alt="<?=$v['ten'.$lang]?>"></p>
			            <div class="info-news">
		...
		/*--- Lấy thuvien  ----*/
		function lay_thuvien($truyvan,$popup=0){
				global $lang,$sluglang;
				...
				foreach($truyvan as $k => $v){ 
					...
					$sql="select photo from #_gallery where hienthi=1 and id_photo =".$v['id']."  and com='product' and type = '".$v['type']."' and kind='man' and val = '".$v['type']."' order by stt,id desc";
			    	$arr_cap=array();
			   	 	$hinhanhsp = $this->d->rawQuery($sql,$arr_cap);
			   	 	
					?>
					<div class="album">
						<a class=" text-decoration-none" data-fancybox="gallery<?=$v['id']?>" href="<?=UPLOAD_PRODUCT_L.$v['photo']?>" data-caption="<?=$v['ten'.$lang]?>">
				            <p class="pic-album scale-img"><img onerror="this.src='<?=THUMBS?>/300x350x1/assets/images/noimage.png';" src="<?=THUMBS?>/300x350x1/<?=UPLOAD_PRODUCT_L.$v['photo']?>" alt="<?=$v['ten'.$lang]?>"/></p>
				            <h3 class="name-album text-split"><span><?=$v['ten'.$lang]?></span></h3>
				        </a>

			       <?php foreach($hinhanhsp as $k2 => $v2){ ?>

						<a class=" text-decoration-none none" data-fancybox="gallery<?=$v['id']?>" href="<?=UPLOAD_PRODUCT_L.$v2['photo']?>" data-caption="<?=$v['ten'.$lang]?>">
				            <p class="pic-album scale-img"><img onerror="this.src='<?=THUMBS?>/300x350x1/assets/images/noimage.png';" src="<?=THUMBS?>/300x350x1/<?=UPLOAD_PRODUCT_L.$v2['photo']?>" alt="<?=$row_detail['ten'.$lang]?>"/></p>
				        </a>
				    <?php } ?>
					</div>
				<?php  } }else{
				...
				foreach($truyvan as $k => $v){ ?>
					<a class="album text-decoration-none" href="<?=$v[$sluglang]?>" title="<?=$v['ten'.$lang]?>">
			            <p class="pic-album scale-img"><img onerror="this.src='<?=THUMBS?>/300x350x1/assets/images/noimage.png';" src="<?=THUMBS?>/300x350x1/<?=UPLOAD_PRODUCT_L.$v['photo']?>" alt="<?=$v['ten'.$lang]?>"/></p>
			            <h3 class="name-album text-split"><?=$v['ten'.$lang]?></h3>
			        </a>			            
		<?php } }				
		}
		/*--- Lấy thuvien  ----*/
		...
		/*--- Lấy video ----*/
		function lay_video($truyvan){
				global $lang,$sluglang;
								
				foreach($truyvan as $k => $v){ ?>
					<a class="video text-decoration-none" data-fancybox="video" data-src="<?=$v['link_video']?>" title="<?=$v['ten'.$lang]?>">
			            <p class="pic-video scale-img"><img onerror="this.src='<?=THUMBS?>/480x360x2/assets/images/noimage.png';" src="https://img.youtube.com/vi/<?=$this->getYoutube($v['link_video']);?>/0.jpg" alt="<?=$v['ten'.$lang]?>"/></p>
			            <h3 class="name-video text-split"><?=$v['ten'.$lang]?></h3>
			        </a>	            
		<?php }				
		}
		/*--- Lấy video ----*/
				...
				//$dongdau = WATERMARK.'/product/360x390x1/';
					...
					$str .='<div class="product">
			            <a class="box-product text-decoration-none" href="'.$v[$sluglang].'" title="'.$v['ten'.$lang].'">
			                ...
			                <p class="price-product">';
			                    if($v['giamoi']!=0 && $v['gia']!=0) {
			                        $str .='<span class="price-new">'.number_format($v['giamoi'],0, ',', '.').'đ</span>
			                        <span class="price-old">'.number_format($v['gia'],0, ',', '.').'đ</span>
			                        <span class="price-per">-'.round(100-($v['giamoi']/$v['gia']*100)).'%</span>';
			                        //<span class="price-per">-'.$v['giakm'].'%</span>
			                    } else {
		...
		function get_cap($dieukien,$typesp,$loaitable,$gioihan){
			global $lang;
			$sql="select ten$lang, tenkhongdauvi, tenkhongdauen, id from #_".$loaitable." where type='".$typesp."' and hienthi=1 and ".$dieukien.">0 order by stt asc limit 0,".$gioihan;
	    	$arr_sqlc_num=array();
		...
		function get_product_id($dieukien,$typesp,$loaiid,$iddanhmuc,$gioihan){
			global $lang;
			$sql="select type, id, ten$lang, tenkhongdauvi, tenkhongdauen, mota$lang, noidung$lang, masp, luotxem, id_brand, id_mau, id_size, id_list, id_cat, id_item, id_sub, id_tags, photo, options, giakm, giamoi, gia from #_product where type='".$typesp."' and ".$loaiid."=".$iddanhmuc." and hienthi=1 and ".$dieukien.">0 order by stt asc limit 0,".$gioihan;
	    	$arr_cap=array();	    	
		...
		function get_product($dieukien,$typesp,$gioihan){
			global $lang;
			$sql="select type, id, ten$lang, tenkhongdauvi, tenkhongdauen, mota$lang, noidung$lang, masp, luotxem, id_brand, id_mau, id_size, id_list, id_cat, id_item, id_sub, id_tags, photo, options, giakm, giamoi, gia from #_product where type='".$typesp."'  and hienthi=1 and ".$dieukien.">0 order by stt asc limit 0,".$gioihan;
	    	$arr_cap=array();	    	
	   	 	return $truyvan = $this->d->rawQuery($sql,$arr_cap);	   	 	
		}
		...
		function get_baiviet($dieukien,$typesp,$gioihan){
			global $lang;
			$sql="select type, id, ten$lang, tenkhongdauvi, tenkhongdauen, mota$lang, noidung$lang, luotxem, id_tags, photo, ngaytao from #_news where type='".$typesp."' and hienthi=1 and ".$dieukien.">0 order by stt asc limit 0,".$gioihan;
	    	$arr_cap=array();
	   	 	return $truyvan = $this->d->rawQuery($sql,$arr_cap);	   	 	
		}

		/*--- Lấy text ----*/
		function lay_text($cot,$type){
			global $lang,$sluglang;
			
			$sql="select $cot$lang from #_static where hienthi=1 and type='".$type."' ";
	    	$arr_bv=array();
	   	 	$row_detail = $this->d->rawQueryOne($sql,$arr_bv);				
			
			//var_dump($row_detail);
			return $row_detail[$cot.$lang];	
		}
		...
		function lay_mxh($type,$width='',$height='',$zc=2){
			...
			if($width>0 and $height >0){
				$str_thumb = "/".$width."x".$height."x1/";
			}
			
			$sql = "select ten$lang as ten,link,photo from #_photo where hienthi=1 and type='".$type."' order by stt,id desc";
			$arr_cap=array();
			$lkweb = $this->d->rawQuery($sql,$arr_cap);
			//var_dump($lkweb);
			foreach($lkweb as $k => $v){
				$str .= '<a href="'.$v['link'].'" target="_blank"><img src="'.THUMBS.$str_thumb.UPLOAD_PHOTO_L.$v['photo'].'" alt="'.$v['ten'].'" /></a>';
