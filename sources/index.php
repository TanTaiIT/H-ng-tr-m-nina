<?php  
	if(!defined('SOURCES')) die("Error");
    @$id = htmlspecialchars($_GET['id']);

    $cap1 = $func->get_cap('hienthi','san-pham','product_list',2); //danhmuc, list, cat, item ( product/news )
    //$sanphambc = $func->get_product('banchay','san-pham',14);    
	//$popup = $d->rawQueryOne("select ten$lang, photo, link, hienthi from #_photo where type = ? and act = ? limit 0,1",array('popup','photo_static'));
    $slider = $d->rawQuery("select ten$lang, photo, link from #_photo where type = ? and hienthi > 0 order by stt,id desc",array('slide'));
    $brand = $d->rawQuery("select ten$lang, tenkhongdauvi, tenkhongdauen, id, photo from #_product_brand where type = ? and hienthi > 0 order by stt,id desc",array('san-pham'));
    $pronb = $d->rawQuery("select id from #_product where type = ? and noibat > 0 and hienthi > 0",array('san-pham'));
    /*$splistnb = $d->rawQuery("select ten$lang, tenkhongdauvi, tenkhongdauen, id from #_product_list where type = ? and noibat > 0 and hienthi > 0 order by stt,id desc",array('san-pham'));*/
    
    $newsnb = $d->rawQuery("select ten$lang, tenkhongdauvi, tenkhongdauen, mota$lang, ngaytao, id, photo, options from #_news where type = ? and noibat > 0 and hienthi > 0 order by stt,id desc",array('tin-tuc'));
    $videonb = $d->rawQuery("select id from #_photo where noibat > 0 and type = ? and hienthi > 0",array('video'));
    $partner = $d->rawQuery("select ten$lang, link, photo from #_photo where type = ? and hienthi > 0 order by stt, id desc",array('doitac'));
    $dichvu=$d->rawQuery("select ten$lang,tenkhongdau$lang,id,photo from #_news where type = ? and hienthi > 0 and noibat > 0 order by stt, id desc",array('dich-vu'));
    $goidichvu=$d->rawQuery("select ten$lang,tenkhongdau$lang,id,photo from #_news where type = ? and hienthi > 0 and noibat > 0 order by stt, id desc limit 0,6",array('goi-dich-vu'));
 
    // $row_detail1 = $d->rawQueryOne("select type, id, ten$lang, tenkhongdauvi, tenkhongdauen, mota$lang, noidung$lang, masp, luotxem, id_brand, id_mau, id_size, id_list, id_cat, id_item, id_sub, id_tags, photo, options, giakm, giamoi, gia from #_product where id = ? and type = ? and hienthi > 0 limit 0,1",array($id,$type));
    $felling = $d->rawQuery("select ten$lang, mota$lang,id,photo,options2 from #_news where type = ? and hienthi > 0 and noibat > 0 order by stt,id desc",array('cam-nhan'));

    $why = $d->rawQuery("select ten$lang, mota$lang,id,photo from #_news where type = ? and hienthi > 0 and noibat > 0 order by stt,id desc limit 0,4",array('why'));

    $kienthuc=$d->rawQuery("select ten$lang,tenkhongdau$lang,mota$lang,id,photo,ngaytao from #_news where type= ? and hienthi > 0 and noibat > 0 order by stt,id desc",array('kien-thuc'));


   
  



    //$get_danhmuc = $func->get_cap('hienthi','san-pham','product_list',2); //danhmuc, list, cat, item ( product/news )
    //$get_sanpham = $func->get_product('banchay','san-pham',14); 
    //$get_baiviet = $d->rawQuery("select ten$lang, tenkhongdauvi, tenkhongdauen, mota$lang, ngaytao, id, photo, options from #_news where type = ? and noibat > 0 and hienthi > 0 order by stt,id desc",array('tin-tuc'));
    //$get_1bv = $d->rawQueryOne("select ten$lang, noidung$lang from #_static where type = ? limit 0,1",array('footer'));
    //$get_1photo = $d->rawQueryOne("select id, photo, options from #_photo where type = ? and act = ? limit 0,1",array('logo','photo_static'));
    //lay du lieu dang noi : htmlspecialchars_decode($static['noidung'.$lang])
    
    /* SEO */
    $seoDB = $seo->getSeoDB(0,'setting','capnhat','setting');
    if(!empty($seoDB['title'.$seolang])) $seo->setSeo('h1',$seoDB['title'.$seolang]);
    if(!empty($seoDB['title'.$seolang])) $seo->setSeo('title',$seoDB['title'.$seolang]);
    if(!empty($seoDB['keywords'.$seolang])) $seo->setSeo('keywords',$seoDB['keywords'.$seolang]);
    if(!empty($seoDB['description'.$seolang])) $seo->setSeo('description',$seoDB['description'.$seolang]);
    $seo->setSeo('url',$func->getPageURL());
    $img_json_bar = (isset($logo['options']) && $logo['options'] != '') ? json_decode($logo['options'],true) : null;
    if($img_json_bar == null || ($img_json_bar['p'] != $logo['photo']))
    {
        $img_json_bar = $func->getImgSize($logo['photo'],UPLOAD_PHOTO_L.$logo['photo']);
        $seo->updateSeoDB(json_encode($img_json_bar),'photo',$logo['id']);
    }
    if(count($img_json_bar) > 0)
    {
        $seo->setSeo('photo',$config_base.THUMBS.'/'.$img_json_bar['w'].'x'.$img_json_bar['h'].'x2/'.UPLOAD_PHOTO_L.$logo['photo']);
        $seo->setSeo('photo:width',$img_json_bar['w']);
        $seo->setSeo('photo:height',$img_json_bar['h']);
        $seo->setSeo('photo:type',$img_json_bar['m']);
    }
?>